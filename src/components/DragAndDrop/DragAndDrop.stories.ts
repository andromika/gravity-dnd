...
export const StressTest: Story = {
  render: () => ({
    setup() {
      const slots = ref<(DemoItem | null)[]>(Array.from({ length: SLOT_COUNT }, () => null));
      const pool = ref<DemoItem[]>(
        Array.from({ length: 1000 }, (_, i) => ({ id: `item-${i + 1}`, label: `Item ${i + 1}` })),
      );
    ...
    components: { DragAndDrop, Draggable, Slot, Pool },
    template: `
      <div style="width: 720px; max-width: 96vw; font-family: system-ui, sans-serif;">
        <div style="margin-bottom: 12px; font-weight: 700;">Stress Test: Pool 1000 items</div>
        <DragAndDrop>
          <div style="display: grid; gap: 16px;">
            <div>
              <div style="margin-bottom: 8px; font-size: 14px; font-weight: 600;">Slots</div>
